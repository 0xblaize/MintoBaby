...
export default function LoginPage() {
  ...
  const handleAdminLogin = async (event: React.FormEvent) => {
    ...
    setAdminError('');
    try {
      const response = await fetch(`${API_BASE}/auth/admin`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: adminEmail.trim(), password: adminPassword }) });
      const body = await response.json().catch(() => ({}));
      ...
      setAdminError(error instanceof Error ? error.message : 'Administrator sign-in failed.');
    } finally {
  ...
  return (
    <main style={{ position: 'fixed', inset: 0, zIndex: 9999, isolation: 'isolate', background: '#0d0d12', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, boxSizing: 'border-box', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' }}>
      <section style={{ width: 'min(560px, 100%)', maxHeight: 'calc(100vh - 48px)', overflow: 'hidden', boxSizing: 'border-box', background: '#14131a', border: '1px solid rgba(107,60,232,0.4)', borderRadius: 20, padding: 36, color: '#f5f5f5', boxShadow: '0 25px 60px rgba(0,0,0,0.85)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: '#827e99', fontSize: 11, margin: '24px 0 16px' }}><span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.1)' }} /> ADMINISTRATOR ACCESS <span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.1)' }} /></div>
        <form onSubmit={handleAdminLogin}><input aria-label="Administrator email" type="email" required value={adminEmail} onChange={(event) => setAdminEmail(event.target.value)} placeholder="Administrator email" style={{ width: '100%', boxSizing: 'border-box', marginBottom: 10, padding: '12px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: '#1c1b24', color: '#fff' }} /><input aria-label="Administrator password" type="password" required value={adminPassword} onChange={(event) => setAdminPassword(event.target.value)} placeholder="Administrator password" style={{ width: '100%', boxSizing: 'border-box', marginBottom: 10, padding: '12px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: '#1c1b24', color: '#fff' }} />{adminError && <div style={{ color: '#ff4d73', fontSize: 12, marginBottom: 10, textAlign: 'center' }}>{adminError}</div>}<button type="submit" disabled={adminLoading} style={{ width: '100%', padding: 12, borderRadius: 10, border: '1px solid rgba(255,255,255,0.15)', background: '#1c1b24', color: '#fff', fontWeight: 700 }}>{adminLoading ? 'Signing in...' : 'Sign in as Administrator'}</button></form>
