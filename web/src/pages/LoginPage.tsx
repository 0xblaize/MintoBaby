...
import { IconX, IconCheck, IconBolt, IconLock, IconCreditCard } from '../components/MintoIcons';

export default function LoginPage() {
  const navigate = useNavigate();
  // Auth context not strictly needed here since we handle session directly below
  
  const [googleError, setGoogleError] = useState('');
  const [googleLoading, setGoogleLoading] = useState(false);
  const [step, setStep] = useState<'auth' | 'admin_pass' | 'payment' | 'success'>('auth');
  
  // Default to Pro plan and weekly billing as they were the defaults in the modal
  ...
  return (
    <div
      style={{
        ...
        inset: 0,
        zIndex: 9999,
        background: '#0d0d12', // Pure static dark background
        display: 'flex',
        alignItems: 'center',
      ...
      >
        ...
        {step === 'auth' && (
          <div>
            ...
            {googleError && (
              <div style={{ padding: '12px', background: 'rgba(245,80,80,0.1)', border: '1px solid rgba(245,80,80,0.3)', borderRadius: '8px', color: '#ff4d73', fontSize: 13, marginBottom: '16px', textAlign: 'center' }}>
                {googleError}
              </div>
            )}
        ...
        {step === 'payment' && (
          <div>
            ...
            {/* Promo Code Section */}
            {!promoApplied && (
              <div style={{ marginBottom: 20, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 12 }}>
                <form onSubmit={handleClaimPromoCode} style={{ display: 'flex', gap: 8 }}>
                  <input
                    type="text"
                    placeholder="Enter Promo Code (e.g. minto2026)"
                    value={promoInput}
                ...
                {promoStatus && (
                  <div style={{ marginTop: 8, fontSize: 11, fontWeight: 600, color: promoStatus.success ? '#00ff88' : '#ff4d73' }}>
                    {promoStatus.message}
                  </div>
                )}
              </div>
